...
void AirPlace::onNormalTick(LocalPlayer* localPlayer) {
    ...
    if(hitResult->type == HitResultType::AIR) {
        ItemStack* carriedItem = localPlayer->getsupplies()->container->getItem(
            localPlayer->getsupplies()->getmSelectedSlot());
        if(carriedItem == nullptr)
            return;

        if(carriedItem && carriedItem->isBlock()) {
            ...
            if(GetAsyncKeyState(VK_RBUTTON) & 0x8000) {
                if(shouldPlace) {
                    if(localPlayer->gamemode != nullptr)
...
void AirPlace::onLevelRender() {
    ...
    MCR::drawBox3dFilled(renderAABB, UIColor(color.r, color.g, color.b, opacity),
