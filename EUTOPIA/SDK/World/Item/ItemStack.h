...
public:
    ...
    void reinit(Item* item, int count, int itemData) {
      mVfTable = reinterpret_cast<uintptr_t**>((uintptr_t)MemoryUtil::getVtableFromSig("48 8D 05 ? ? ? ? 48 89 85 ? ? ? ? 44 89 A5 ? ? ? ? C6 85 ? ? ? ? ? 0F B6 05"));
      MemoryUtil::callVirtualFunc<void>(3, this, item, count, itemData);
  }
    //48 8D 35 ? ? ? ? 48 8B 87
  static ItemStack* getEmptyItem() {
      return reinterpret_cast<ItemStack*>((uintptr_t)MemoryUtil::getVtableFromSig("48 8D 35 ? ? ? ? 48 8B 87"));
  }

  //MemoryUtil::getFuncFromCall(MemoryUtil::findSignature("48 89 5C 24 10 48 89 7C 24 18 55 48 8D AC 24 30 FF FF FF 48 81 EC D0 01 00 00 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 85 C0 00 00 00 48 8B D9 E8")
  //float getDestroySpeed(Block* block) {
  //    using func_t = float(__thiscall*)(ItemStack*, Block*);
  //    static func_t Func = reinterpret_cast<func_t>(MemoryUtil::getFuncFromCall(MemoryUtil::findSignature("48 89 5C 24 10 48 89 7C 24 18 55 48 8D AC 24 30 FF FF FF 48 81 EC D0 01 00 00 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 85 C0 00 00 00 48 8B D9 E8")));
  //    return Func(this, block);
  //}

  std::string getCustomName() {
    std::string str;
    static auto address = MemoryUtil::findSignature("48 89 ? ? ? 57 48 83 EC ? 48 8B ? 48 8B ? E8 ? ? ? ? 84 C0 74 ? 48 8B ? 48 8B ? E8 ? ? ? ? 48 8B");
...
std::string getEnchantName(Enchant enchant) const {
    static const char* names[] = {
        ...
        "AQUA_AFFINITY", "SHARPNESS", "SMITE", "BANE_OF_ARTHROPODS",
        "KNOCKBACK", "FIRE_ASPECT", "LOOTING", "EFFICIENCY",
        "SILK_TOUCH", "UNBREAKING", "FORTUNE", "POWER",
        "PUNCH", "FLAME", "BOW_INFINITY", "LUCK_OF_THE_SEA",
        "LURE", "FROST_WALKER", "MENDING", "BINDING",
        "VANISHING", "IMPALING", "RIPTIDE", "LOYALTY",
        "CHANNELING", "MULTISHOT", "PIERCING", "QUICK_CHARGE",
        "SOUL_SPEED", "SWIFT_SNEAK"
    };
    
    std::string name = (static_cast<int>(enchant) < 38) ? names[static_cast<int>(enchant)] : "UNKNOWN";
