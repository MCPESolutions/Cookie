...
struct BoxRotation {
    ...
    float rotY;
    float rotZ;
    BoxRotation(float _x = 0.f, float _y = 0.f, float _z = 0.f) : rotX(_x), rotY(_y), rotZ(_z) {}
};
...
void drawCylinder3d(const Vec3<float>& baseCenter, float radius, float height, const UIColor& color,
                    int segments);
void drawSphere3d(const Vec3<float>& center, float radius, const UIColor& color, int segments);
void drawSphere3dFilled(const Vec3<float>& center, float radius, const UIColor& color, int stacks,
                        int slices);
...
void drawWings(const Vec3<float>& center, float yaw, float wingSpan, float animationTime,
               const UIColor& color, const UIColor& lineColor);
}  